...
<!-- rationale: ADR-0001 + ADR-0035 — 外部更新との diff に必要な origin schema と探索パス、
     および skill-creator への命令形配線（ADR-0018 の 40% 上限 fallback、2026-08-22）を常駐 -->
<!-- review-when: origin 管理または skill discovery path を変えた時 /
     skill-creator の自発発火が 40% を安定して超えた時（命令形配線を外す） -->
...
（入口の intent 確認と fresh-context の草稿ゲートを持つ。自発発火は 40% が天井なので命令形で

...
| [0075](0075-cloud-session-as-default-build-executor.md) | build 役の既定を Claude Code cloud session に移す — 起動は `scripts/cloud-dispatch.sh` の 1 本（preflight / `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1` + `--ref` で bundle 回避 / pty）、packet は自己完結（chain を本文に、cloud は実装と built-in `/code-review` medium のみ）、verify は repo の CI（`main` と `claude/**`、SHA pin、secret scan）で判断役は headSha 一致の run を読む。cloud に出さない条件は task-triage §3 の表。公開 repo も対象だが、無人 cycle の cloud dispatch は private repo に限る（public は digest で名指しして人間が OK した task だけ、script は `--public-ok` 無しで拒む — ADR-0043 red line 3 を守る）。PR は view で merge は ff-only。ADR-0043 Decision 2、ADR-0057 Decision 1、ADR-0069 Decision 3 に注記 | accepted | 2026-09-24 |
| [0076](0076-build-proposals-with-reproducer-and-opus-5-5-packet-rules.md) | build の起票は再現手順つき提案まで開く（Report の `Proposed tasks` 最大 2 件、判断役が再現を確かめて digest へ、起票の決定は著者）。Opus 5.5 活用ガイド（claude.dev 2026-09-22）と claude.ai 高速化記事（2026-09-23）の規則を packet の「進め方」と Report（`Needs from judge` / `Model:`）に入れ、task-triage §3 に著者だけが答えを持つ 4 項目の逆提案を置く。cloud の `--model` / `--effort` 固定は未決。ADR-0055 Decision 5 に注記 | accepted | 2026-09-25 |
| [0077](0077-readme-review-single-judge-with-claims-check.md) | readme-writer のレビュー agent を readme-judge 1 本にまとめる（readme-reviewer / readme-clarity-reviewer を退役し、問いを checklist の §F フロア・§J 日本語と言語間・R10 へ移す）。判定器は README だけで答えを凍結してから主張を repo と照合し（Phase B、file:line）、tools は Read / Grep / Glob、model は opus（照合型）。最終判定は fresh 1 回 + 同じ質問セットの recheck 1 回で打ち切る。codex-review は著者が求めたときだけ。ADR-0070 Decision 3・ADR-0033 Decision 2・ADR-0055 に注記 | accepted | 2026-09-25 |
| [0078](0078-generation-audit-single-entry-with-prompt-audit-engine.md) | generation-audit を世代交代監査の単一入口にし、dated pattern 走査を `/claude-api prompt-audit` に委ねる（Phase 3。pattern 表は写さない）。Phase 0 で対象モデル・範囲・適用除外、Phase 1–2 で runtime 照合、Phase 4 で High / Medium を skill-creator §3 の書き方で適用。Opus 5.5 監査の実施記録（所見 107・適用 64、Opus 5.5 固有の型は 0 件）。ADR-0061 の統合却下・ADR-0022 Decision 1・ADR-0073 の update-suppressor 判定に注記 | accepted | 2026-09-25 |
| [0079](0079-one-axis-per-question-and-retire-ask-one-question-hook.md) | 問いの規律を数から観点へ — signal-first の問いの項を「1 問 1 観点、複数の問いは並べてよい」に置き換え、`hooks/ask-one-question.sh`（AskUserQuestion の 2 問以上を block）を退役。観点の混合は機械ゲートにしない。signal-first style は `default` のまま（著者選択）で、規律は memory の 1 行が運ぶ。ADR-0073 の Decision 1・2・Review-when・Consequences に注記 | accepted | 2026-09-25 |
| [0080](0080-review-when-watch-production-first.md) | RFC と ADR の review-when（53 本）を、jev-research-pipeline（jrp、毎日 05:00）が書いた新しいノート 1 本ごとに Jev で 1 組ずつ照合する launchd job（`skills/review-when-watch/`、毎日 06:10 に 1 回）を shadow を経ずに本番で回す。plist は bash の wrapper を起動し、wrapper がノートを iCloud の vault からローカルの stage へ rsync して Python には stage だけを読ませる（TCC）。判定済みはファイル名で記録。通知は `met`（Noul）だけで決め（閾値 0.5 は仮置き）、`overlap`（Choice）は並べ方だけ。初回・7 日無音の週次・失敗も Slack に送り、通知が計器を兼ねる。canary を毎回 1 組、全組の生の読み値をログへ。vault ノートの外部送信は plist の登録をもって著者が許容する。ADR-0074 の shadow-first はこの job に適用しない | accepted | 2026-09-25 |
| [0081](0081-per-packet-effort-and-bounce-classification.md) | build の effort を judge が packet ごとに端のケースの密度で表から選ぶ（docs・設定 = low / 通常の feat = medium（未記入の既定、cloud の実測既定）/ brownfield の fix・parser・並行性・性能・security = high / 無人で長く検証が厳しい = xhigh、max は著者の明示だけ）。作成時の `--effort` は cloud に転送されず作成後の `/effort` は効く（2026-09-26 probe）ので、`cloud-dispatch.sh --effort` は `/effort <level>` で session を作り packet を `-p` で送る 2 段起動にする。bounce は 1 語で分類し、見落としは `/effort` を 1 段上げてから差し戻し、読み違いは effort を上げず仕様を直し、gate・rebase は集計外。digest の build 行に effort / bounce 有無 / 分類。ADR-0075 の effort 記述と ADR-0076 の未決に注記 | accepted | 2026-09-26 |
| [0082](0082-hooklint-fail-open-blocks-and-approved-gate-loss-blocks.md) | hooklint の FAIL_OPEN を report-only から block に昇格（`b522d0d` が 5 件を分類して 0 件にし、`# hooklint: fail-open <理由>` の waiver で意図した素通しを書けるようになったため。出力書式と report-only の枠は保持、golden の末尾行は 9 / 0）。verify-precommit はゲートの有無を `-x` でなく存在で見て、台帳に載っている repo のゲート消失（削除・壊れた symlink・読めない・repo 外 symlink）を block、台帳に無い repo は従来どおり素通し。台帳照会に読み取り専用の `verify_allow.py known`、起動器は hook 相対で解決。harness-lint は lint の異常終了を block・出力を未検証データの枠に入れ・linked worktree からの commit でも発火（検査器は main の版）。3 hook の bypass は文字列全体の先頭の env 代入だけ。ADR-0063 の Decision 1 / 未決 Alternative / 較正 Review-when と ADR-0038 の再配置制約に注記 | accepted | 2026-09-26 |
| [0083](0083-verify-gate-in-linked-worktrees-via-main-checkout-approval.md) | linked worktree の verify.sh を登録先の main worktree の承認で照合して実行する（台帳の key は repo 自身が無ければ、`git worktree list` の登録・管理ディレクトリの指し返し・main の git dir と common dir の一致で確かめた main worktree で引き、照合・実行は worktree 自身のバイト列、経路の包含判定は worktree の root。common dir の親では引かない — 偽の `.git` ファイルで承認済み repo を名乗れるため。main の key で hash が違えば 71 でなく 70 で、案内は merge 後に main で承認。`known` は一致した key を出し、worktree のゲート消失の案内は branch に戻す・`VERIFY_BYPASS` を先に、revoke を merge 後に。approve / revoke と台帳形式は不変。branch の `harness_lint.py` が verify 経由で走ることを受け入れる）。bandit / ruff-format の譲りを `-x` から `-f` に。ADR-0082 の Decision 2 / 3 / 4・Review-when・Follow-ups と ADR-0059 の Decision に注記 | accepted | 2026-09-26 |

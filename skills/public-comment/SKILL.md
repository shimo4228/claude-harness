...
- **concede-first / net-giver の 3 拍**: ①相手の nuance ある論点を具体的に認める → ②根拠ある価値を 1 つ足す（実例・反直感の史実・視覚的類推）→ ③鋭い一文で終える。リンク勧誘・「続きは私の記事で」をしない。見返りを期待しない価値提供が、読者を自発的にプロフィールへ向かわせる（Kaguura 2026 の net-giver 哲学）
...
- **宣伝をしない**: 自作物への言及は議論に必要な場合のみ。リンクは求められてから。DOI・repo 誘導を入れない。「私も同じテーマで書いています、見てください」型のスレッド乗っ取りはその最悪形（削除対象になり信用も失う）

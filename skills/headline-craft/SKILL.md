...
**削る技法（追加ではなく除去）**: ポジティブ形容詞（素晴らしい・強力な・完全な）、ヘッジ（〜について・〜の話・〜メモ）、冗長な前置き、em dash（——）・句点・読点による 2 文連結。削った字数を具体性に回す。

**長さの既定**: 日本語は 20〜36 字、1 文。platform の上限（contract）は天井であって目標ではない。上限に近い候補は、命題を再述している印なので削る。
...
1. **core claim 抽出** — 本文（またはドラフト・要旨）から「読者が持ち帰る 1 つの主張・成果」を 1 文で書き出す。タイトルはこの 1 文を**指し示す**もので、再述ではない。命題文を短くした「A は B。C なのは D」型は候補にしない（本文にない約束をしないための基準は同じ）
